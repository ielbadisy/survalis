...
`compute_*()` / `benchmark*()` functions from 1.x remain available for the 2.x
series; see `vignette("from-1.0", package = "survalis")` and
`?`survalis-deprecated``.
...
`benchmark()` is the single entry point for comparing learners: `tune = FALSE`
(default) runs each with fixed hyperparameters; `tune = TRUE` tunes each
learner internally via nested cross-validation.
