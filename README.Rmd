...
Alongside the seven verbs, `survalis` ships a few task-specific helpers that
operate on the same fitted models and `survmat` predictions:
...
* `roc_survmat()` / `dca_survmat()` -- IPCW time-dependent ROC curve and
  decision-curve analysis at a fixed horizon; `timeroc_survmat()` for the
  AUC(t) trajectory. Each result has a `plot()` method.
